...
void transporate_matrix(figure* tetr) {
  int rows = FIGURE_ROWS - 1, cols = FIGURE_COLS - 1;
  int temp[rows][cols];
  // except for I, only 3 rows need to be flipped for the others
...
void score(GameInfo_t* frame, int lines, int restart) {
  ...
  if (frame->level > 10) frame->level = 10;
...
int record(int record) {
  int err = 0;
  FILE* file = fopen("recordTetris.txt", "w");
  if (file == NULL) {
    err = 1;
...
int read_record(GameInfo_t* frame) {
  FILE* file;
  int err = 0;
  file = fopen("recordTetris.txt", "r");
  if (file == NULL) {
    file = fopen("recordTetris.txt", "w+");
    if (file != NULL) {
      fprintf(file, "%d", 0);
  ...
  if (fscanf(file, "%d", &(frame->high_score)) != 1) {
    file = freopen("recordTetris.txt", "w", file);
    fprintf(file, "%d", 0);
    fseek(file, 0, SEEK_SET);
  } else {
    err = fscanf(file, "%d", &(frame->high_score));
    if (err == 0) printf("Command failed with code %d", err);
  }
  if (fclose(file) != 0) {
    perror("Ошибка закрытия файла");
    err = 1;
...
void shift(GameInfo_t* frame, figure* tetr, struct timespec* last_fall) {
  ...
  if ((now.tv_sec - last_fall->tv_sec) +
      ...
      (0.8 / (frame->speed * 1.0))) {
    if (collision_with_sides(frame, tetr) != 0) tetr->x = tetr->last_x;
    if (attaching(frame, tetr) == 0) tetr->y += 1;
