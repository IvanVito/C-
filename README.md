...
Этот репозиторий содержит три проекта: **matrixCPP** (Матрицы), **copyContainers** (Контейнеры) и **snake** (Змейка). Цель этих проектов — продемонстрировать использование C++ для реализации основных структур данных, алгоритмов и игр с объектно-ориентированным подходом. Каждый проект включает исчерпывающие модульные тесты для обеспечения корректности реализации. Подробности по каждому проекту приведены ниже.
...
Проекты в этом репозитории демонстрируют реализацию различных аспектов программирования на C++, включая работу с матрицами, реализацию стандартных контейнеров и создание игр. Каждый проект обеспечен тестами и документацией для поддержки надежности и удобства использования.
