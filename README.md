...
Этот репозиторий содержит два проекта: `matrixCPP (Матрицы)` и `copyContainers (Контейнеры)`. Цель этих проектов — реализовать основные структуры данных и алгоритмы, используя C++ с объектно-ориентированным подходом, а также предоставить исчерпывающие модульные тесты для каждой реализации. `Больше информации о проектах можно найти в соответсвующих папках проектов.`
